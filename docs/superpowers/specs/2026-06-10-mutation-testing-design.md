...
1. **Text-level authoring, AST-level validation.** Mutants are
   authored as plain text replacements by an LLM or the user — not
   generated by an AST operator engine. But SqlProof already ships
   `pglast` (parse + `RawStream` deparse, used in
   `schema/parse_sql.py`), so the harness parses every mutated body
   before spending a database on it: a mutant that doesn't parse is an
   authoring error, never a kill. SqlProof provides the part that must
   be deterministic: locate, apply, validate, run, report, score.
...
- After application, the mutated body is parsed with `pglast` (already
  a dependency). A body that fails to parse is an authoring error; a
  body whose AST equals the original's is rejected as a no-op mutant.
  Both checks run before any database is cloned.
- The target function body is located via the existing
  `schema/parse_sql.py` parse of the schema, not by regex over the
  DDL file.
...
- **LLM-proposed mutants.** Given a function body, an LLM proposes
  ...
  sketch.
- **A small static catalog** (operator swaps, `AND` ↔ `OR`, JOIN-type
  swaps, `FILTER` drops) as a "what might you be missing" pass.
  Because `pglast` parse + `RawStream` deparse are already in-tree,
  these are mechanical AST transforms — days of work, not the months
  the original issue assumed for a from-scratch engine. Still not v1,
  but a near-term follow-up rather than a maybe-never; it emits the
  same JSON mutant format as every other authoring layer.
...
- A broad AST-level mutation operator engine (the small static catalog
  above is the ceiling; anything semantic is the LLM layer's job).
- Automatic equivalent-mutant detection beyond the AST no-op check.
...
- Mutant identity for score tracking over time: hash of
  `(target, normalized AST of mutated body)` — pglast/libpg_query
  fingerprinting makes identity robust to formatting changes, so this
  is mostly settled; renaming a function still orphans its history.
