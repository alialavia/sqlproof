...
<Note>Application fails loudly if `old` is absent or appears more than once — a mutant that didn't apply must never count as killed.</Note>

### `Drop(pattern)`

Delete exactly one occurrence of `pattern` from the body.

<Note>Same absent/ambiguous rules as `Replace`.</Note>
...
<Note>`expect_survives=True` requires a `reason=`; a `reason=` without
...
cover it); delete the stale declaration.</Note>
...
the clone, and drops the clone (`WITH (FORCE)`).

<Note>There is no restore path by design — the template is never touched.</Note>
