...
<Note>
`db.savepoint()` is used around the UPDATE so that if the WITH CHECK raises a policy-violation error, the transaction stays open for the post-state read. Without it the aborted transaction would prevent the verification query from running.
</Note>
...
See also [Missing DELETE policy](/examples/inbox/missing-delete-policy) — the sibling write-side RLS bug.
