...
<Steps>
  <Step title="Keep pgTAP for the boundary cases">
    Fixed-value assertions that document specific behavior decisions ("yes, we round half-to-
    even on JPY", "yes, unknown promos are silently ignored"). pgTAP
    is excellent prose-form for "this is the exact answer for this
    exact input."
  </Step>
  <Step title="Add SqlProof for the invariants">
    Monotonicity, idempotency, bounded-output, equivalence. Shorter to write, catches more.
  </Step>
  <Step title="Use SqlProof to generate the next pgTAP fixture">
    When SqlProof's shrinker reports a counterexample, port it into a pgTAP
